...
<style>
  ...
  #main-image { max-width: 80%; max-height: 500px; margin: 20px auto; border: 2px solid #007bff; border-radius: 8px; }
  #gallery { display:flex; overflow-x:auto; margin-top:20px; padding:10px; background-color:#e9ecef; border-radius:5px; }
  .thumb { margin:0 5px; cursor:pointer; transition: transform 0.2s; border: 2px solid #dee2e6; border-radius:4px; }
  .thumb:hover { transform: scale(1.1); border-color:#007bff; }
</style>
</head>
<body>
...
<div class="container my-3">
  <h1 class="mb-3">CAVISCOPE PICTURE MANAGER</h1>

  <div class="alert alert-primary">
    <i class="fa fa-exclamation-triangle"></i> It takes 5-10 seconds to capture a photo.
  </div>

  <div class="mb-3">
    <button class="btn btn-primary me-2" onclick="rotatePhoto();">
      <i class="fa fa-undo"></i> ROTATE PIC
    </button>
  ...
  <div class="text-center">
    <img src="saved-photo" id="photo" alt="Captured photo" class="img-fluid" id="main-image">
  </div>

  <!-- Filmstrip gallery -->
  <div id="gallery" class="mt-3">
    <!-- Example placeholders, replace with dynamically added images -->
    <!-- <img src="saved-photo1.jpg" class="thumb" onclick="showMain(this)"> -->
    <!-- <img src="saved-photo2.jpg" class="thumb" onclick="showMain(this)"> -->
  </div>
</div>
...
<script>
  ...
  function capturePhoto() {
    fetch("/capture")
      .then(response => console.log("Photo capture requested"))
      .catch(err => console.error(err));
  }
